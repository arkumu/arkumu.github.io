...
| Nicht-öffentlicher Name (Begründung) | Non-public Name (Reason) | 0-1 | ✓<br/>(linebreak separated; one for each non-public name) | | | Eine interne Begründung, weswegen der oder die Namen nicht veröffentlicht werden können | An internal reason why the name(s) cannot be published | | |
...
| [ORCID](/documentation/data-model/graph-model#orcid) | [ORCID](/documentation/data-model/graph-model#wikidata-id) | 0-1 | ⨉ | | | Die ORCID des/der Akteur:in | The ORCID of the Actor | | |
| [Wikidata-ID](/documentation/data-model/graph-model#wikidata-id) | [Wikidata ID](/documentation/data-model/graph-model#wikidata-id) | 0-1 | ⨉ | | | Wikidata-ID des/der Akteur:in | Wikidata ID of the Actor | | |
...
| [Dateipfad](/documentation/data-model/graph-model#file-path) | [File Path](/documentation/data-model/graph-model#file-path) | 1<br/>(handled automatically) | ⨉ | | | Der Dateipfad der Datei im arkumu.nrw-System | The file path of the file in the arkumu.nrw system | Simultan auch der originale relative Ordnerpfad der Datei, der für die Metadaten verwendet wird | Simultaneously, the original relative folder path of the file used for the metadata |
| [Dateiname](/documentation/data-model/graph-model#file-name) | [File Name](/documentation/data-model/graph-model#file-name) | 1<br/>(handled automatically) | ⨉ | | | Der originale Dateiname der hochgeladenen Datei | The original file name of the uploaded file | |
| [Dateigröße](/documentation/data-model/graph-model#file-size) | [File Size](/documentation/data-model/graph-model#file-size) | 1<br/>(handled automatically) | ⨉ | | | Die Dateigröße der hochgeladenen Datei | The file size of the uploaded file | | |
...
| [MIME Type](/documentation/data-model/graph-model#mime-type) | [MIME Type](/documentation/data-model/graph-model#mime-type) | 1<br/>(handled automatically) | ⨉ | | | Der MIME Type der Datei, beim Ingest automatisch ausgelesen | The MIME type of the file, automatically extracted during ingest | | |
| Dateipaket | File Package | 0/1 | ⨉ | | | Eine Angabe, ob es sich bei der Datei um ein Dateiaket handelt. Wenn keine Auswahl getroffen wurde, so heißt dies nicht automatisch, dass es sicht nicht um ein Dateipaket handelt | An indication of whether the file is a file package. If no selection has been made, this does not automatically mean that it is not a file package | Dieses feld kann technisch unterstützt werden | This field could be automised |
| Erhaltungstyp | Preservation Type | 1 | ⨉ | | | Eine Angabe, um was für eine Art Datei es sich im archivarischen Sinne handelt, entwerder "Preservation Master", "Modified Master" oder "Derivate Copy" (Nutzer:innenkopie) | An indication of what type of file it is in an archival sense, either “Preservation Master,” “Modified Master,” or "Derivate Copy" (user copy) | Könnte bei Bedarf auch eine eigene Entität sein | Could also be a separate entity if necessary |
| Derivatkopie-Nummer | Derivative Copy Number | [0-1]<br/>(if Preservation Type is "Derivate Copy") | ⨉ | | | Eine Angabe zu welchem Derivat-Dateiset sie gehört | Information about which derivative file set it belongs to | In dieser Iteration des Datenmodells optional gemacht | Made optional in this version of the data model |
| Medientyp | Media Type | 1 | ⨉ | many-to-one<br/><sub>(each Digital Object is assigned exactly one Media Type; each Media Type can be used for many Digital Objects)</sub> | [Media Type](#media-type) | Eine grobe Medien-Kategorisierung der Datei, z.B. Bild, Text, Audio, Code, etc. | A rough media categorisation of the file, e.g., Image, Text, Audio, Code, etc. | |
| Entstehungstyp | Genesis Type | 0-1 | ⨉ | many-to-one<br/><sub>(each Digital Object can be assigned to one Gesis Type; each Genesis Type can be used for many Digital Objects)</sub> | [Genesis Type](#media-type) | Eine grobe Kategorisierung zur Entstehung der Datei | A rough categorisation of the file's origin | Derzeit "Retrodigitalisat" oder "Born Digital" | Currently "Retro-Digitised" or "Born Digital" |
| [Digitales-Objekt-Lizenz](/documentation/data-model/graph-model#digital-object-licence) | [Digital Object Licence](/documentation/data-model/graph-model#digital-object-licence) | 1 | ⨉ | many-to-one<br/><sub>(each Digital Object must be assigned exactly one License; each License may be linked to many Digital Objects)</sub> | [Digital Object License](#digital-object-license) | Die zentrale Lizenzangabe zur Datei. Entweder eine arkumu-Lizenz oder eine externe Lizenz, z.B. von Creative Commons | The central licence statement for the file. Either an arkumu licence or an external licence, e.g. from Creative Commons | | |
...
| ist arkumu Preview | is arkumu Preview | 0-1 | ⨉ | | | Eine Angabe, ob eine Datei als Vorschau auf der Projekt-Detailseite angezeigt werden soll. Dabei sind Lizenz- und Anzeigeeinschränkungen zu beachten. Insbesondere bei großen Bildern kann es sinnvoll sein, auf eine direkte Anzeige für Benutzer:innen zu verzichten, um unnötigen Datenverbrauch zu vermeiden | An indication of whether a file should be displayed as a preview on the Project's details page. License and display restrictions must be observed. Especially with large images, it may be advisable to refrain from displaying them directly to Users in order to avoid unnecessary data usage. | |
| [Digitales-Objekt-Schlagwort](/documentation/data-model/graph-model#digital-object-keyword) | [Digital Object Keyword](/documentation/data-model/graph-model#digital-object-keyword) | 0-n | ✓<br/>(multi select) | many-to-many<br/><sub>(each Digital Object can have multiple Keywords; each Keyword may be linked to multiple Digital Objects)</sub> | [Keyword](#keyword)<br/>(see comment) | Ein Schlagwort zur Datei | A keyword for the file | | |
| [Deutsche Inhaltliche Beschreibung](/documentation/data-model/graph-model#german-content-description) | [German Content Description](/documentation/data-model/graph-model#german-content-description) | 0-1 | ⨉ | | | Eine deutsche Beschreibung des Dateiinhalts | A description of the file contents in German | | |
| [Englische Inhaltliche Beschreibung](/documentation/data-model/graph-model#english-content-description) | [English Content Description](/documentation/data-model/graph-model#english-content-description) | 0-1 | ⨉ | | | Eine englische Beschreibung des Dateiinhalts | A description of the file contents in English | | |
| [Deutsche Bildbeschreibung](/documentation/data-model/graph-model#german-image-description) | [German Image Description](/documentation/data-model/graph-model#german-image-description) | 0-1 | ⨉ | | | Speziell für Bilddateien, eine deutsche Beschreibung des Bildinhaltes, die auch zum Bild angezeigt wird. Vorrangig auch benutzbar zur Zugänglichmachung der Bilder für Menschen mit eingeschränkten Sehen | Specifically for image files, a description of the image content in German, which is also displayed alongside the image. Primarily used to make images accessible to people with impaired vision | | |
| [Englische Bildbeschreibung](/documentation/data-model/graph-model#english-image-description) | [English Image Description](/documentation/data-model/graph-model#english-image-description) | 0-1 | ⨉ | | | Speziell für Bilddateien, eine deutsche Beschreibung des Bildinhaltes, die auch zum Bild angezeigt wird. Vorrangig auch benutzbar zur Zugänglichmachung der Bilder für Menschen mit eingeschränkten Sehen | Specifically for image files, a description of the image content in German, which is also displayed alongside the image. Primarily used to make images accessible to people with impaired vision | | |
...
| [Systemvoraussetzungen](/documentation/data-model/graph-model#system-requirements) | [System Requirements](/documentation/data-model/graph-model#system-requirements) | [1]<br/>(for Media Type "Code") | ⨉ | | | Eine Angabe, welche technische Umgebung für das ausführen der Dateien benötigt wird | Information about the technical environment required to execute the files | | |
...
| [Sprachversion](/documentation/data-model/graph-model#language-version) | [Language Version](/documentation/data-model/graph-model#language-version) | 0-n | ✓<br/>(multi select) | many-to-many<br/><sub>(each IDigital Object may hold multiple Language Version; each Language can be linked to multiple Digital Objects)</sub> | [Language](#language) | Die Sprachfassung beschreibt die Sprache der in einem Digitalen Objekt zusätzlich enthaltenen Tonspuren. Sie gibt an, in welcher Sprache der meist gesprochene Inhalt wiedergegeben wird (z. B. als Tonspur einer synchronisierten Fassung). | The Language Version describes the language of additional audio tracks contained in a Digital Object. It specifies the language in which the mostly spoken content is presented (e.g. as the audio track of a dubbed version) | | |
| [JHOVE-Dateistatus](/documentation/data-model/graph-model#jhove-status) | [JHOVE Status](/documentation/data-model/graph-model#jhove-status) | 1<br/>(handled automatically) | ⨉ | | | Eine aus JHOVE ausgelesene Information, ob die Datei valide und wohlgeformt ist | Information extracted from JHOVE indicating whether the file is valid and well-formed | | |
| [DROID-PUID] | [DROID-PUID] | 1<br/>(handled automatically) | ⨉ | | | Eine ID der Datei zu ihrem Eintrag in der PRONOM-Dateiformat-Datenbank | An ID for the file corresponding to its entry in the PRONOM file format database | | |
| MediaInfo-Track-Format | MediaInfo Track Format | 1<br/>(handled automatically) | ⨉ | | | Eine Angabe aus MediaInfo extrahiert. Das MediaInfo-XML enthält im Feld media-track-format eine bis viele Angaben zu den in den Tracks der Datei benutzten Codecs- bzw. Formate, z.B. "JPEG", "PCM", "JPEG 2000", "MPEG-4", "PNG", "TIFF", "PDF", "AVC", "YUV", "AC-3", "AAC", "DCP", "ProRes", "QuickTime", etc. Diese sollen normalisiert gespeichert werden und durchsuchbar sein | Information extracted from MediaInfo. The MediaInfo XML contains one or more entries in the media-track-format field regarding the codecs or formats used in the tracks of the file, e.g., “JPEG,” “PCM,” “JPEG 2000,” “MPEG-4,” “PNG,” “TIFF,” “PDF,” “AVC,” “YUV,” “AC-3,” “AAC,” “DCP,” “ProRes,” “QuickTime,” etc. These should be stored in a normalized format and be searchable | | |
...
| Technische Dauer | Technical Duration | 1<br/>(for audio and video; automatically handled) | ⨉ | | | Die technisch ausgelesene Dauer des Dateiinhalts | The technical duration of the file content | | |
| Projektkomplation | Project Compilation | 0/1<br/>(enum) | ⨉ | | |  "ja", "nein", oder keine Aussage (leere Auswahl), ob die Datei mehrere Projekte enthält | "yes", “no", or no statement (blank selection) indicating whether the file contains multiple Projects | | |
| Teil einer Serie | Part of a Series | 0/1 | ⨉ | | | Eine Angabe, ob die Datei, bzw. ihr Inhalt, Teil einer Serie von Dateien oder Inhalten ist | An indication of whether the file or its contents are part of a series of files or contents | | |
| Tonmischfassung | Dubbing Version | 0-1 | ⨉ | | | Freitext; z.B. "Fernsehmischung", "Kinomischung" | Free text; e.g., “TV Mix,” “Cinema Mix” | |
| Tonformat | Sound Format | 0-1 | ⨉ | | | Eine Auswahl aus einer Auswahlliste, z.B. "Mono", "Stereo" | A selection from a list, e.g. "Mono", "Stereo" | | |
| DCP-Art | DCP Type | 0-1 | ⨉ | | | Eine Auswahl aus einer Auswahlliste: "CleanFeed / Originalversion", "Burnt-in Subtitles", "Supplemental Subtitles" | A selection from a  list: "CleanFeed / Originalversion", "Burnt-in Subtitles", "Supplemental Subtitles" | | |
| EQ | EQ | 0-1 | ⨉ | | | Angabe zur Equalizerkurve im Freitext |Information about the equalizer curve in Free Text | | |
| wird im Loop abgespielt | plays in a Loop | 0-1<br/>(for audio and video) | ⨉ | | | Angabe, ob die Wiedergabe der Datei im Frontend automatisch wiederholt werdem soll | Indication whether the file should be automatically repeated in the frontend | | |
| EQ | EQ | 0-1 | ⨉ | | | Angabe zur Equalizerkurve im Freitext |Information about the equalizer curve in Free Text | | |
...
| Wählt automatisch folgende Digitales-Objekt-Lizenz aus | Pre-selects following Digital Object Licence  | 0-1 | ⨉ | many-to-one<br/><sub>(each Existing Licence Agreement may preset a Digital Object to one existing Digital Object Licence; the same Digital Object Licence may be used by multiple Existing Licence Agreements)</sub> | [Digital Object Licence](#digital-object-licence) | Eine Digitales-Objekt-Lizenz, die automatisch für alle Digitalen Objekte voreingestellt werden sollte, wenn die entpdrechenden Objekte in einem Ereignis hochgeladen werden, dass von dem verknüpften Projekt aus erstellt wurde | A Digital Object Öicence that should be automatically preset for all Digital Objects when the corresponding objects are uploaded in an Event created from the linked Project | | |
...
| [Englische Synonyme](/documentation/data-model/graph-model#english-synonym) | [English Synonyms](/documentation/data-model/graph-model#english-synonym) | 0-1 | ✓<br/>(comma separated) | | | Eine Reihe von kommagetrennten englischen Synonymen der Rolle | A series of comma-separated English synonyms of the Role | | |
...
| Erstellt in Nutzer:innen-Rolle | Created in User Role | 1<br/>(handled automatically) | ⨉ | <sub>Derived from User at time of creation</sub> | Derived from [User](#user) | Die Benutzer:innen-Rolle der erstspeichernden Person | The User Role of the person who first saved the data record | | | 
...
| Zuletzt geändert in Nutzer:innen-Rolle | Last modified in User Role | 1<br/>(handled automatically) | ⨉ | <sub>Derived from User at time of last modification</sub> | Derived from [User](#user) | Die Benutzer:innen-Rolle der aktuell speicehernden Person | The User Role of the person who last saved the data record | | |
