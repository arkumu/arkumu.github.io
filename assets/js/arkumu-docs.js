...
document.addEventListener("DOMContentLoaded", function() {
  ...
  tables.forEach(function(table) {
    ...
    // Zentriert den Inhalt von Link-Spalten //
    // Centers the content of link columns //
    ...
    if (rows.length > 1) {
      ...
      for (let colIndex = 0; colIndex < colCount; colIndex++) {
        let allLinksOrEmpty = true;

        for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
          const cell = rows[rowIndex].cells[colIndex];
          if (cell) {
            const children = cell.children;
            const isLink = children.length === 1 && children[0].tagName === 'A';
            const isEmpty = cell.textContent.trim() === '';
            if (!isLink && !isEmpty) {
              allLinksOrEmpty = false;
              break;
            }
          }
        }
        ...
        if (allLinksOrEmpty) {
          if (rows[0].cells[colIndex]) {
            rows[0].cells[colIndex].style.textAlign = 'center';
          }
          for (let row of rows) {
